...
eMBErrorCode eMBRegInputCB(UCHAR* pucRegBuffer, USHORT usAddress, USHORT usNRegs)
{
	...
	for (uint16_t i = 0; i < usNRegs;) {
		...
		bool res = modbusHandleReadInputRegister(usAddress + i - 1, value);
...
eMBErrorCode eMBRegHoldingCB(UCHAR* pucRegBuffer, USHORT usAddress, USHORT usNRegs, eMBRegisterMode eMode)
{
	...
	if (eMode == MB_REG_READ) {
		...
		for (uint16_t i = 0; i < usNRegs;) {
			ModbusReturnValue value = { .buffer = &wordPtr[i], .pIdx = &i };
			bool res = modbusHandleReadHoldingRegister(usAddress + i - 1, value);
#ifdef MODBUS_MISSING_AS_ZERO
			if (!res)
				value.U16(0);
#else
		...
		for (uint16_t i = 0; i < usNRegs; i++) {
			...
			if (!modbusHandleWriteHoldingRegister(usAddress + i - 1, ModbusValue::U16(value)))
				return MB_ENOREG;
		}
#endif
	}
