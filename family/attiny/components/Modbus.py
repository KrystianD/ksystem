...
from family.attiny.config import ModbusConfig
from generator.IStatementsContainer import IStatementsContainer


class ModbusComponent(IComponent):
    def __init__(self, cfg: ModbusConfig, interrupts: InterruptsComponent, modbus_serial: SerialComponent):
        self.cfg = cfg
        self.interrupts = interrupts
        self.modbus_serial = modbus_serial

        self.interrupts.register_handler(self.modbus_serial.get_interrupt_RXC(),
                                         body="pxMBFrameCBByteReceived()")
        self.interrupts.register_handler(self.modbus_serial.get_interrupt_DRE(),
                                         body="pxMBFrameCBTransmitterEmpty()")
    ...
    def emit_internal_header(self, source_file):
        source_file.add(cgen.Define(f"MODBUS_SERIAL", self.modbus_serial.name))

        if self.cfg.rs485_dir_pin is not None:
            raise Exception("not implemented with custom DIR pin")

        if self.cfg.missing_as_zero is True:
            source_file.add(cgen.Define(f"MODBUS_MISSING_AS_ZERO", 1))

        cnt = 0
        for func_name in self.cfg.functions:
    ...
    def get_additional_source_files(self) -> List[str]:
        return ["../../third-party/freemodbus/demo/AVR/port/portevent.c",
                ...
                "../../third-party/freemodbus-attiny-port/portserial.cpp",
                "../../third-party/freemodbus-attiny-port/porttimer.cpp",
                "../../third-party/freemodbus-attiny-port/mbcrc.c",

                "library/kModbus.cpp"]

    def get_additional_header_directories(self) -> List[str]:
        return ["../../third-party/freemodbus-attiny-port/",
